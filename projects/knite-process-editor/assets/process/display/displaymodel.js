...
///local storage start
var payload = {}
//alert(modelEditorJson);

var processData = window.localStorage.getItem("process-data");
if(processData) {
    var processDataArr = JSON.parse(processData);
    var processDetails = processDataArr.find(e => e.id == modelId);
    var processModelDataArr = window.localStorage.getItem("process-model-data");
    if(processModelDataArr) {
        var processModelData = JSON.parse(processModelDataArr);
        var processModel = processModelData.find(e => e.key === processDetails.key);
        payload.modelEditorJson = processModel.json_xml;
    }
}
...
var request = jQuery.ajax({
    ...
    url: "http://localhost:8081/api/v1/process/model-json",
