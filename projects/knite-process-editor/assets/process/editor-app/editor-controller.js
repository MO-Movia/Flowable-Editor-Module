...
angular.module('flowableModeler')
    ...
    //       if(processModelData) {
    //         var processModelData = JSON.parse(processModelData);
    //         var processModel = processModelData.find(processModel => processModel.key === process.key);
    //         //data.data.model = JSON.parse(processModel.json_xml);
    //         data.data.model = JSON.parse(sessionStorage.getItem("editor-json"));
    //       }
	...
	// });
    var promise = new Promise((resolve, reject) => {
        var processEditorJson = window.sessionStorage.getItem("process-editor-json");
        if(processEditorJson) {
            var process = JSON.parse(processEditorJson);
            var data = {
                "data": {
                    "modelId": process.id,
                    "name": process.name,
                    ...
                    "lastUpdated": "",
                    "lastUpdatedBy": "admin",
                    "model": process.editorJson
                }
            }
            resolve(data);
        } else {
            reject("Error: session storage process editor json is not present")
        }
    });
	promise.then(response => {
        ...
        if(modelData.data.model.stencilset.namespace == 'http://b3mn.org/stencilset/cmmn1.1#') {
           ...
           return $http.get(FLOWABLE.URL.getDmnStencilSet());
              ...
              return $http.get("./assets/process/mock-data/bpmn-editor.json");
          }
